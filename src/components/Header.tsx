  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-40 backdrop-blur-md bg-background/80 border-b border-border/50">
      <div className="container max-w-6xl flex items-center justify-between py-3 px-4">
        ...
        >
          <img src={LOGO} alt="Дом Мечты" className="h-12 md:h-16 w-auto" />
        </Link>
      ...
      {/* Mobile dropdown */}
      {menuOpen && (
        <div className="md:hidden bg-background/95 backdrop-blur-md border-b border-border/50 px-4 pb-5 pt-2 flex flex-col gap-2">
          {isHome ? (
            <>
              <a href="#services" onClick={closeMenu} className="px-4 py-3 rounded-2xl bg-accent text-accent-foreground text-sm font-medium text-center">Услуги</a>
              <a href="#about" onClick={closeMenu} className="px-4 py-3 rounded-2xl bg-accent text-accent-foreground text-sm font-medium text-center">О нас</a>
              <a href="#contacts" onClick={closeMenu} className="px-4 py-3 rounded-2xl bg-accent text-accent-foreground text-sm font-medium text-center">Контакты</a>
              <Link to="/commercial" onClick={closeMenu} className="px-4 py-3 rounded-2xl bg-terracotta/15 text-terracotta text-sm font-semibold text-center">Коммерция</Link>
              <Link to="/for-client" onClick={closeMenu} className="px-4 py-3 rounded-2xl bg-muted text-[#565166] text-sm font-medium text-center">Клиентам</Link>
            </>
          ) : (
            <>
              <button onClick={() => { navigate('/'); closeMenu(); }} className="px-4 py-3 rounded-2xl bg-accent text-accent-foreground text-sm font-medium text-center flex items-center justify-center gap-2">
