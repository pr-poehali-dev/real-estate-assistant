      ...
      <div style={{ display:'grid', gridTemplateColumns:'auto 1fr', gap:'clamp(32px,5vw,72px)', alignItems:'start' }} className="hero-grid">
        ...
        <div className="animate-fade-up hero-badges-col" style={{ display:'flex', flexDirection:'column', gap:12, paddingTop:8, animationDelay:'200ms' }}>
          {[
            ...
            <span key={b.label} className="hero-badge" style={{ display:'flex', alignItems:'center', gap:10, fontFamily:'Inter,sans-serif', fontSize:11, fontWeight:400, letterSpacing:'0.12em', textTransform:'uppercase', color:MUTED, padding:'14px 18px', lineHeight:1, alignItems:'center', border:`1px solid ${GOLD20}`, borderRadius:100, background:'hsl(222 30% 10%)' }}>
              <Icon name={b.icon} size={13} style={{ color:GOLD, flexShrink:0, display:'block' }} />
              <span style={{ display:'block', paddingTop:1 }}>{b.label}</span>
            </span>
          ))}
        </div>
